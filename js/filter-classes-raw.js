...
class ModalFilterClasses extends ModalFilter {
	...
	async pGetUserSelection ({filterExpression = null, selectedClass = null, selectedSubclass = null, isDisabled = false} = {}) {
		...
		return new Promise(async resolve => {
			...
			this._filterCache.list.items.forEach(li => li.data.tglSel.classList.toggle("disabled", this._isDisabled));

			// region Restore selection
			if (selectedClass != null) {
				const ixClass = this._filterCache.allData.findIndex(it => it.name === selectedClass.name && it.source === selectedClass.source);
				const ixSubclass = ~ixClass ? this._filterCache.allData[ixClass].subclasses.findIndex(it => it.name === selectedSubclass.name && it.source === selectedSubclass.source) : -1

				if (~ixClass) {
					ModalFilterClasses._doListDeselectAll(this._filterCache.list);

					const clsItem = this._filterCache.list.items.find(it => it.data.ixClass === ixClass && it.data.ixSubclass == null);
					if (clsItem) {
					...
					if (~ixSubclass && clsItem) {
						const scItem = this._filterCache.list.items.find(it => it.data.ixClass === ixClass && it.data.ixSubclass === ixSubclass);
		...
		if (this._filterCache) {
			...
			allData.forEach((it, i) => {
				...
				filterListItems.forEach((filterListItem, i) => {
					...
					filterListItem.ele.addEventListener("click", evt => {
						if (this._isDisabled) return;
						ModalFilterClasses.handleSelectClick(list, filterListItems, filterListItem, i === 0, evt);
					});
				});
			});
